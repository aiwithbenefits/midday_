-- Update global_search function to use correct tsquery syntax
DROP FUNCTION IF EXISTS global_search(text, uuid, text, int, int, numeric);
...
BEGIN
  ...
  (
    ...
    SELECT 
      ...
      COALESCE(t.name, 'Transaction')::text as title,
      CASE 
        WHEN search_term IS NULL OR search_term = '' THEN 0.0
        ELSE ts_rank(t.fts_vector, plainto_tsquery(language_param::regconfig, search_term))::float
      END as relevance,
    ...
    WHERE t.team_id = team_id_param
      AND (
        search_term IS NULL 
        OR search_term = ''
        OR t.fts_vector @@ plainto_tsquery(language_param::regconfig, search_term)
      )
  ...
  (
    ...
    SELECT 
      ...
      CASE 
        WHEN search_term IS NULL OR search_term = '' THEN 0.0
        ELSE ts_rank(i.fts, plainto_tsquery(language_param::regconfig, search_term))::float
      END as relevance,
      i.created_at,
      jsonb_build_object(
        'id', i.id,
    ...
    WHERE i.team_id = team_id_param
      AND (
        search_term IS NULL 
        OR search_term = ''
        OR i.fts @@ plainto_tsquery(language_param::regconfig, search_term)
      )
    ORDER BY relevance DESC
    LIMIT items_per_table_limit
  )
